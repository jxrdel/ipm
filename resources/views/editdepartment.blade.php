...
@section('title')
    <title>ICT Contracts | Department Details</title>
@endsection
            ...
            <div class="card-body">
                    ...
                    <div class="col">
                        ...
                        <select name="BusinessGroupTypeId" class="form-select" style="display: inline;width: 150px">
                            ...
                            @foreach ($bgtypes as $bgtype)
                                <option value="{{ $bgtype->ID }}"
                                    {{ $department->BusinessGroupTypeId == $bgtype->ID ? 'selected' : '' }}>
                                    {{ $bgtype->Name }} </option>
                            @endforeach
                ...
                <div class="row" style="padding-top: 20px">
                        ...
                        <select name="ParentId" class="form-select" style="display: inline;width: 350px">
                            ...
                            @foreach ($alldepts as $dept)
                                <option value="{{ $dept->ID }}"
                                    {{ $department->ParentId == $dept->ID ? 'selected' : '' }}>{{ $dept->Name }}
                                </option>
                            @endforeach
                    ...
                    <div class="col">
                        ...
                        <input style="display: inline; margin-left:10px" class="form-check-input" type="checkbox"
                            name="IsActive" {{ $department->IsActive == 1 ? 'checked' : '' }}>
                    </div>
                </div>
            </div>
        </div>
