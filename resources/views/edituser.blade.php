...
@section('title')
    <title>ICT Contracts | Edit User</title>
@endsection
    ...
    <form method="POST" id="updateUser" action="{{ route('updateuser') }}">
        ...
        <div class="row">
            ...
            <div class="col" style="text-align: end">
                ...
                <a href="{{ route('userdetails', ['id' => $user->ID]) }}" class="btn btn-outline-secondary"
                    style="width: 7rem">Cancel</a>
                        ...
                        <div style="margin-top: -6px">
                            ...
                            <select name="InternalContactId" class="form-select" style="display: inline;width: 300px">
                                ...
                                @foreach ($internalcontacts as $internalcontact)
                                    <option value="{{ $internalcontact->ID }}"
                                        {{ $selectedIC == $internalcontact->ID ? 'selected' : '' }}>
                                        {{ $internalcontact->FirstName }} {{ $internalcontact->LastName }}</option>
                                @endforeach
                ...
                <div class="card-body">
                    <div class="col" style="text-align: center;padding-bottom:20px">
                        <form method="POST" id="addrole" action="{{ route('addrole', ['id' => $user->ID]) }}">
                            ...
                            <button type="submit" class="btn btn-primary" style="width: 8rem"><i class="fas fa-plus"></i>
                                Add Role</button>
                        </form>
                    </div>
                    @if (count($roles) > 0)
                                ...
                                @foreach ($roles as $role)
                                    <tr>
                                        ...
                                        <td style="text-align: center"><a
                                                href="{{ route('deleterole', ['id' => $role->ID, 'userid' => $user->ID]) }}"><button
                                                    type="button" class="btn btn-outline-danger"><i
                                                        class="bi bi-trash"></i></button></a></td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                            ...
                            <tbody>
                                @foreach ($permissiongroups as $permissiongroup)
                                    <tr>
                                        ...
                                        <td style="text-align: center"><a
                                                href="{{ route('deletepgroup', ['id' => $permissiongroup->ID, 'userid' => $user->ID]) }}"><button
                                                    type="button" class="btn btn-outline-danger"><i
                                                        class="bi bi-trash"></i></button></a></td>
                                    </tr>
                                @endforeach
