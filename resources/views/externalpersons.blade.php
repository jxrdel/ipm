...
@section('title')
    <title>IPM | External Persons</title>
@endsection
            ...
            <div class="card-body">
                ...
                @auth
                    @if (auth()->user()->hasPermission('ExternalContactPerson : Create : Screen'))
                        <a type="button" data-bs-toggle="modal" data-bs-target="#createModal" class="btn btn-primary btn-icon-split" style="width: 10rem">
...
@endif
    <script>
        ...
        function showView(id) {
            Livewire.dispatch('show-viewep-modal', { id: id });
        }
        
        function showEdit(id) {
            var hasPermission = "{{auth()->user()->hasPermission('ExternalContactPerson : Edit : Screen') ?? ''}}";
            if (hasPermission == 1){
                Livewire.dispatch('show-editep-modal', { id: id });
            } else {
