...
@endif
    <script>
        $(document).ready(function() {
            $('#ictable').DataTable({
                ...
                "columns": [
                        ...
                        { data: 'PhoneExt', name: 'PhoneExt' },
                        { data: 'DepartmentName', name: 'BusinessGroups.Name' },
                        { data: 'RoleName', name: 'MOHRoles.Name' },
                        {
                            data: null,
                            orderable: false,
                            searchable: false,
        ...
        function showView(id) {
            Livewire.dispatch('show-viewic-modal', { id: id });
        }
        
        function showEdit(id) {
            var hasPermission = "{{auth()->user()->hasPermission('InternalContact : Edit : Screen') ?? ''}}";
            if (hasPermission == 1){
                Livewire.dispatch('show-editic-modal', { id: id });
            } else {
        ...
        function showDelete(id) {
            ...
            if (hasPermission == 1){
                ...
                if (deleteConfirmation) {
                    // User clicked OK
                    Livewire.dispatch('delete-user', { id: id });
                }
            } else {
                $('#deniedModal').modal('show');
            }
    ...
    <script>
        ...
        window.addEventListener('show-edit-success', event => {
            ...
            toastr.success("Contact edited successfully",'' , {timeOut:3000});
        })
        ...
        window.addEventListener('show-delete-success', event => {
            ...
            toastr.success("Contact deleted successfully",'' , {timeOut:3000});
        })
    </script>
