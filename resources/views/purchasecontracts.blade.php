...
@section('title')
    ...
    <title>ICT Contracts | Purchase Contracts</title>
@endsection
                ...
                <div class="col">
                    @auth
                        @if (auth()->user()->hasPermission('PurchaseContract : Create : Screen'))
                            <a href="{{ route('purchasecontracts.create') }}" class="btn btn-primary btn-icon-split"
                                style="width: 12rem">
...
@section('scripts')
    ...
    <script>
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            return '<a href="#" onclick="showView(' + data.ID +
                                ')">View</a> | <a href="PurchaseContracts/Edit/' + data.ID +
        ...
        function showView(id) {
            Livewire.dispatch('show-viewpc-modal', {
                id: id
            });
        }
        ...
        function showEdit(id) {
            ...
            if (hasPermission == 1) {
                Livewire.dispatch('show-editpc-modal', {
                    id: id
                });
            } else {
    ...
    <script>
        ...
        new MultiSelectTag('internalcontacts', {
            onChange: function(values) {
                console.log(values)
                Livewire.dispatch('set-internalcontacts', {
                    values: values
                })
            }
        }) // id
